...
router.post(
  ...
  asyncHandler(async (req, res): Promise<void> => {
    try {
      ...
      if (err instanceof ValidationError || err instanceof DatabaseError) throw err;
...
router.delete(
  ...
  asyncHandler(async (req, res): Promise<void> => {
    try {
      const id = parseIntStrict(req.params.id, "Customer ID");
      const result = await db.delete(customersTable).where(eq(customersTable.id, id));
      if (!result) {
        throw new NotFoundError("Customer");
      }
      res.sendStatus(204);
...
router.get(
  ...
  asyncHandler(async (req, res): Promise<void> => {
    try {
      ...
      if (err instanceof ValidationError || err instanceof NotFoundError) throw err;
      logger.error(err, "Failed to fetch customer history");
      throw new DatabaseError("Failed to fetch customer history");
    }
  })
);
